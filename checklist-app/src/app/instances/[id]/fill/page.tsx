...
export default function FillChecklistPage() {
      ...
      return (
        <div key={node.id} className={`mb-3 p-4 bg-white border rounded-lg ${exception ? 'border-red-500 bg-red-50' : ''}`}
          ...
          {exception && (
            <button
              onClick={async () => {
                const title = prompt('Corrective action title:') || `Exception on ${node.title}`;
                const desc = prompt('Description / corrective steps:') || node.exceptionAction || '';
                const res = await fetch('/api/actions', {
                  method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
                  body: JSON.stringify({ instanceId: id, nodeId: node.id, title, description: desc, priority: 'HIGH' }),
                });
                if (res.ok) flash('Corrective action created');
                else flash('Failed to create action');
              }}
  ...
  const scorePct = (instance as any).maxScore ? Math.round(((instance as any).score || 0) / (instance as any).maxScore * 100) : null;
  const passed = (instance as any).passed;
  const flagged = (instance as any).flaggedCount || 0;
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            {scorePct !== null && (
              <span className={`px-2 py-1 rounded text-xs ${passed === true ? 'bg-green-100 text-green-700' : passed === false ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
                Score {scorePct}% {passed === true ? '✓ Pass' : passed === false ? '✗ Fail' : ''}
              </span>
            )}
            {flagged > 0 && (
              <span className="px-2 py-1 rounded text-xs bg-amber-100 text-amber-700">⚠ {flagged} flagged</span>
            )}
