...
import { Plus, Loader2, Trash2, Send, CheckCircle2, XCircle, RotateCcw, ArrowRight } from 'lucide-react';
...
export default function AdminInstancesPage() {
  ...
  async function reject(id: string) {
    ...
    const res = await fetch(`/api/instances/${id}/reject`, {
      ...
      credentials: 'include', body: JSON.stringify({ reviewComments: comments }),
    });
              ...
              {instances.map((i) => (
                <tr key={i.id} className="border-t hover:bg-gray-50">
                  <td className="p-3 font-medium text-gray-900">
                    <Link href={`/instances/${i.id}/fill`} className="hover:text-indigo-600">{i.template.name}</Link>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-right whitespace-nowrap">
                    <Link href={`/instances/${i.id}/fill`} className="inline-block px-2 py-1 text-xs text-indigo-600 hover:underline mr-2">Open</Link>
