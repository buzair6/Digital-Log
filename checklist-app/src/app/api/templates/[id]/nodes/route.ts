...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const node = await prisma.checklistNode.create({ data: { templateId: params.id, parentNodeId: parentNodeId ?? null, orderIndex: orderIndex ?? 0, title, nodeType, inputType: inputType ?? null, options: options ?? null, isRequired: isRequired ?? false, helpText: helpText ?? null, depthLevel } });
