...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const nodes = await prisma.checklistNode.findMany({ where: { templateId: params.id } });
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  const instanceCount = await prisma.checklistInstance.count({ where: { templateId: params.id } });
  if (instanceCount > 0) {
    const old = await prisma.checklistTemplate.findUnique({ where: { id: params.id }, include: { nodes: true } });
  ...
  const updated = await prisma.checklistTemplate.update({ where: { id: params.id }, data: { name, description, defaultRoutingGroupId, isActive } });
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  // soft delete
  const updated = await prisma.checklistTemplate.update({ where: { id: params.id }, data: { isActive: false } });
  return NextResponse.json({ template: updated });
}
